...
const BookDetails = () => {
  ...
  useEffect(() => {
    const fetchBook = async () => {
      try {
        const res = await authApi.get(`/books/${id}`);
        setBook(res.data);
      } catch (err) {
  ...
  const submitRating = async (value) => {
    ...
    try {
      const res = await authApi.post(`/books/${id}/rate`, { rating: value });
      setUserRated(true);
      setBook((prev) => ({
        ...prev,
  ...
  const submitComment = async (e) => {
    ...
    try {
      const res = await authApi.post(`/books/${id}/comments`, { text: commentText });
      setBook((prev) => ({
        ...prev,
